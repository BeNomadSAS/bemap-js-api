...
- **Survives archive rebuilds (v2.0.1)**: when BeNomad republishes a tileset under the same name, the library appends a per-map version token (`?v=`, discovered from `/api/maps` after login) so the browser fetches fresh bytes instead of mixing month-old cached slices — and an ETag guard self-heals even a tab that was already open during the swap. No blank map, no manual cache clear. Details: [Browser cache → Archive versioning](#page-../docs/browser-cache.md).
- **WMS-only geoservers don't break the basemap**: `geoserver` also feeds the tiles-map resolution (`tilesFile → geoserver → 'default'`), but some geoservers (e.g. `herehlp`) exist only for WMS/services. When the name was merely **inherited from `geoserver`** and the Worker's catalog says no such tiles map exists, the map automatically falls back to the server **default** map (a console note explains it) instead of erroring with 403s. An **explicit `tilesFile` pin never falls back** — a misspelled or unauthorised tileset keeps surfacing its clean 403, as documented below.
