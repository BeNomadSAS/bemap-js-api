...
function shouldIntercept(request) {
  ...
  if (!urlMatchesHost(request.url)) return false;
  // Range-aware: we care about PMTiles (range) and `.pbf` tile fetches
  var isPmtiles = /\.pmtiles($|\?)/.test(request.url) && request.headers.has('Range');
  var isTile = /\/\d+\/\d+\/\d+\.pbf($|\?)/.test(request.url);
  if (!isPmtiles && !isTile) return false;
        ...
        return fetch(request).then(function (response) {
          if (response.status === 200 || response.status === 206) {
            ...
            clone.arrayBuffer().then(function (body) {
              ...
              return cache.put(key, toStore).then(function () { return evictIfNeeded(cache); });
            }).catch(function () { /* ignore caching failures */ });
          }
          return response;
        });
      });
